...
type User struct {
	...
	// PassHash holds the value of the "pass_hash" field.
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case user.FieldUserEmail, user.FieldUserName, user.FieldPassHash:
...
func (u *User) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case user.FieldPassHash:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field pass_hash", values[i])
			} else if value.Valid {
